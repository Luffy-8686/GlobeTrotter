...
async function run() {
  const cities = await prisma.city.findMany({ include: { activities: true } });
  for (const city of cities) {
    const replacementActs = specificActivities[city.name] || [
    ...
    // Update existing activities instead of deleting
    for (let i = 0; i < Math.min(city.activities.length, replacementActs.length); i++) {
      const actId = city.activities[i].id;
      const newAct = replacementActs[i];
